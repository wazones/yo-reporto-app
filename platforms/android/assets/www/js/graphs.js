var graphsController = {
    ...
    loadGoogleApi: function () {
        google.load("visualization", "1.0", {packages: ["corechart"], callback: onChartsReady});
    },
    onChartsReady: function () {
        this.fillDeptos();
    },
    fillDeptos: function () {
        this.deptosMap = new Map;
        var wsUrl = "http://www.gestiondelriesgo.gov.co/ServicioApp/EventosComunidad.asmx/Departamentos?";
        $.get(wsUrl)
            .done(function (data, status, req) {
                processSuccessDepts(data, status, req);
            })
            .fail(function (error) {
                alert('Error: ' + error);
            })
